...
// --- IMPORTANT: Google Sheets & Data Backend Setup ---
// This app uses a Google Apps Script to act as a backend for storing data in Google Sheets.

// --- Step 1: Deploy the Google Apps Script ---
// 1. Create a new Google Sheet in your Google Drive.
// 2. Create six sheets (tabs) and name them exactly: 'Syllabus', 'Tests', 'Classes', 'Todos', 'Logs', 'WeeklySummaries'.
// 3. In the menu, go to Extensions > Apps Script.
// 4. Delete existing code in `Code.gs` and paste the entire Apps Script code block provided below.
// 5. Click "Deploy" > "New deployment".
// 6. Click the gear icon ("Select type") and choose "Web app".
// 7. Fill in the details:
//    - Description: "Study Planner Backend"
//    - Execute as: "Me"
//    - Who has access: "Anyone" (This is crucial for the API to be public).
// 8. Click "Deploy", authorize the script, and grant it permissions (you may need to click "Advanced" > "Go to... (unsafe)").
// 9. Copy the "Web app URL" and paste it into the `GOOGLE_SHEETS_WEB_APP_URL` constant below.

export const GOOGLE_SHEETS_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbyWFCZ_31vXZof081an49R6MTE72C6PpjYZ5GoaIOhfT82oZO-jUe7I80Y5QtbnND69/exec';
...
function doPost(e) {
  ...
  try {
    ...
    let dataSheet;
    if (action === 'saveSyllabus') dataSheet = sheet.getSheetByName('Syllabus');
    else if (action === 'saveTests') dataSheet = sheet.getSheetByName('Tests');
    else if (action === 'saveClasses') dataSheet = sheet.getSheetByName('Classes');
    else if (action === 'saveTodayTodos') dataSheet = sheet.getSheetByName('Todos');
    else if (action === 'saveLogs') dataSheet = sheet.getSheetByName('Logs');
    else if (action === 'saveWeeklySummaries') dataSheet = sheet.getSheetByName('WeeklySummaries');
    else throw new Error('Invalid save action specified.');
...
export const isSheetsConfigured = () => GOOGLE_SHEETS_WEB_APP_URL && GOOGLE_SHEETS_WEB_APP_URL.startsWith('https://');
...
const fetchData = async (action: string, mockData: any) => {
    ...
    try {
        ...
        if (result.success) {
            if (result.data === null && mockData !== null) {
