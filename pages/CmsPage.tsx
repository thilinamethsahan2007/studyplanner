...
const SyllabusManager: React.FC<Pick<CmsPageProps, 'syllabus' | 'subjects' | 'onSyllabusChange'>> = ({ syllabus, subjects, onSyllabusChange }) => {
    ...
    const [combinedModeCms, setCombinedModeCms] = useState<'pure' | 'applied'>('pure');
    ...
    if (selectedSubjectId === 'combined') {
        const syllabusIdToFind = combinedModeCms === 'pure' ? 'combined' : 'combined-applied';
        currentSyllabus = syllabus.find(s => s.subjectId === syllabusIdToFind);
    } else {
        currentSyllabus = syllabus.find(s => s.subjectId === selectedSubjectId);
    }
    ...
    return (
        <div>
            <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                {selectedSubjectId === 'combined' && (
                    <div className="flex-shrink-0 self-end">
                         <ToggleSwitch 
                            options={combinedToggleOptions} 
                            value={combinedModeCms} 
                            onChange={(val) => setCombinedModeCms(val as 'pure' | 'applied')}
                        />
                    </div>
                )}
            </div>
