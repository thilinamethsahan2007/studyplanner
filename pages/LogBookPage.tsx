...
const LogBookPage: React.FC<LogBookPageProps> = ({ logs, onLogsChange, weeklySummaries, subjects }) => {
    ...
    const subjectTotals = React.useMemo(() => {
        return academicSubjects.map(subject => {
            ...
            const totalMinutes = subjectLogs.reduce((sum, log) => sum + log.durationMinutes, 0);
            return {
                ...subject,
                totalMinutes,
            };
        }).filter(s => s.totalMinutes > 0);
    }, [logs, academicSubjects]);
        ...
        <div className="max-w-4xl mx-auto space-y-12">
            ...
            <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-sm">
                ...
                {subjectTotals.length > 0 && (
                    <div className="mb-8">
                        <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200 mb-3">Subject Breakdown</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {subjectTotals.map(subject => (
                                <div key={subject.id} className="p-4 rounded-lg text-center" style={{ backgroundColor: `${subject.color}20` }}>
                                    <p className="font-semibold" style={{ color: subject.color }}>{subject.name === 'Combined Maths' ? 'Maths' : subject.name}</p>
                                    <p className="text-3xl font-bold tracking-tight" style={{ color: subject.color }}>{formatDuration(subject.totalMinutes)}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                                            ...
                                            <div key={categoryKey}>
                                                ...
                                                {categoryLogs.map(log => {
                                                    ...
                                                    <div key={log.id} className="pt-2 pl-4 relative">
                                                        ...
                                                        <div className="flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400 self-end sm:self-center">
                                                            ...
                                                            <button 
                                                                ...
                                                                className="p-2 text-slate-400 hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-400 rounded-full transition-colors"
                                                                aria-label={`Edit log for ${log.todoItemTitle}`}
                                                                title="Edit Log"
                                                            >
